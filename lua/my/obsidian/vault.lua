...
function Vault:new_linked_note(...)
  ...
  assert(dst_note, "src_note must be provided")

  local dst_link = opts.link_fmt:format(dst_note:format_link())
  local src_line = src_note:insert_text(dst_link, opts.src_insert_opts)
  assert(src_line > 0, "Failed to insert link into source note")

  if opts.src_buf then
    local src_col = dst_link:len() + 1
    local item = { tagname = dst_note.id, from = { opts.src_buf, src_line, src_col, 0 } }
  ...
  dst_note:open({
    callback = vim.schedule_wrap(function()
      ...
      local dst_line = dst_note:insert_text(src_link, opts.dst_insert_opts)
